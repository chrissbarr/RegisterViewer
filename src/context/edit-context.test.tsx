...
describe('saveDraft', () => {
  it('returns the draft and removes it from state', () => {
    const { result } = renderEditContext();
    const reg = makeRegister({ id: 'reg-1', name: 'REG' });

    act(() => result.current.enterEditMode(reg));

    let saved: ReturnType<typeof result.current.saveDraft>;
    act(() => {
      saved = result.current.saveDraft('reg-1');
    });

    expect(saved!).toBeDefined();
    expect(saved!.name).toBe('REG');
    expect(result.current.getDraft('reg-1')).toBeUndefined();
  });
  ...
  it('returns undefined for a non-existent draft', () => {
    ...
    let saved: ReturnType<typeof result.current.saveDraft>;
    act(() => {
      saved = result.current.saveDraft('nonexistent');
    });

    expect(saved!).toBeUndefined();
  });

  it('sets isEditing to false when last draft is saved', () => {
    const { result } = renderEditContext();
    const reg = makeRegister({ id: 'reg-1' });
    ...
    act(() => result.current.saveDraft('reg-1'));
    expect(result.current.isEditing).toBe(false);
  });

  it('keeps isEditing true when other drafts remain', () => {
    const { result } = renderEditContext();
    const reg1 = makeRegister({ id: 'reg-1' });
    const reg2 = makeRegister({ id: 'reg-2' });
    ...
    act(() => result.current.saveDraft('reg-1'));
    expect(result.current.isEditing).toBe(true);
    expect(result.current.getDraft('reg-2')).toBeDefined();
  });
});
...
describe('discardDraft', () => {
  it('removes the draft from state', () => {
    ...
    act(() => result.current.enterEditMode(reg));
    act(() => result.current.discardDraft('reg-1'));

    expect(result.current.getDraft('reg-1')).toBeUndefined();
  });

  it('sets isEditing to false when last draft is discarded', () => {
    const { result } = renderEditContext();
    const reg = makeRegister({ id: 'reg-1' });
...
describe('discardAllDrafts', () => {
  it('clears all drafts and sets isEditing to false', () => {
    const { result } = renderEditContext();
    const reg1 = makeRegister({ id: 'reg-1' });
    const reg2 = makeRegister({ id: 'reg-2' });
