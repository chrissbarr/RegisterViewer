            ...
            "api/composer.lock": json.dumps(
                {
                    ...
                    "packages-dev": [
                        {"name": "phpunit/phpunit", "require": {"php": ">=8.3"}},
                    ],
                    "platform": {
                        "php": "^8.3",
                        "ext-curl": "*",
...
jobs:
  ...
  api:
    steps:
      - run: docker compose run --rm test php -r 'if (PHP_MAJOR_VERSION !== 8 || PHP_MINOR_VERSION !== 3) { fwrite(STDERR, "Expected PHP 8.3"); exit(1); }'
  ...
  deploy-payload:
    steps:
      - uses: shivammathur/setup-php@v2
        with:
          php-version: '8.3'
      - run: php -r 'if (PHP_MAJOR_VERSION !== 8 || PHP_MINOR_VERSION !== 3) { fwrite(STDERR, "Expected PHP 8.3"); exit(1); }'
      - run: |
...
""",
    ...
    def test_accepts_consistent_php83_contract(self) -> None:
    ...
    def test_rejects_php84_docker_base(self) -> None:
        ...
        self.assertTrue(any("php:8.3-apache" in error for error in self.run_validation(files)))

    def test_rejects_phpunit13_lockfile(self) -> None:
        files = self.make_files()
        lock = json.loads(files["api/composer.lock"])
        lock["packages-dev"][0] = {"name": "phpunit/phpunit", "require": {"php": ">=8.4.1"}}
    ...
    def test_rejects_missing_doc_contract(self) -> None:
        files = self.make_files()
        files["docs/DEVELOPMENT.md"] = "PHP 8.3+"

        self.assertTrue(any("docs/DEVELOPMENT.md" in error for error in self.run_validation(files)))

    def test_php_requirement_checker_uses_php83_allowance(self) -> None:
        self.assertTrue(php_requirement_allows_target("^8.0"))
        self.assertTrue(php_requirement_allows_target("^7.4 || ^8.0"))
        self.assertTrue(php_requirement_allows_target(">=8.3 <8.4"))
        self.assertTrue(php_requirement_allows_target("!=8.4.* >=8.3"))
        self.assertFalse(php_requirement_allows_target(">=8.4.1"))
